...
class stack_pool {  
    ...
    std::vector<node_t> pool_of_stacks; 

    stack_type free_nodes{ end() }; // at the beginning, it is empty

    node_t& node(stack_type x) noexcept { return pool_of_stacks[x - 1]; } 
    const node_t& node(stack_type x) const noexcept { return pool_of_stacks[x - 1]; }
...
public:
    ...
    */
    void reserve(size_type n) {
        pool_of_stacks.reserve(n); 
    }
    
    /*
    to obtain the capacity of std::vector
    */
    size_type capacity() const { return pool_of_stacks.capacity(); }
    ...
    */
    template<typename X>
    stack_type push(const X& val, stack_type head) { 
        if(!std::is_same<X,value_type>::value){throw DiferentType("Trying to push an invalid type.");}
        return _push(val, head);
    }
    template<typename X>
    stack_type push(X&& val, stack_type head) { 
        if(!std::is_same<X,value_type>::value){throw DiferentType("Trying to push an invalid type.");}
    ...
    */
    stack_type _push(T&& val, const stack_type head) {
        if (empty(free_nodes)) {
            pool_of_stacks.push_back(node_t(val, head,true));
            last(head) = false; 
            return static_cast<stack_type>(pool_of_stacks.size());
        }
        else {
            auto tmp = free_nodes;
    ...
    */
    void print_pool() noexcept {
            ...
            for (auto i = 0; i < pool_of_stacks.size(); i++) {
                std::cout <<"["<< pool_of_stacks[i].value << "]";
            }
            std::cout <<"\n-------------------"<< std::endl;
    }
};
